import { ImportantDateType } from "../types/ImportantDatesTypes";

const importantDates: ImportantDateType[] = [
  {
    date: new Date("2022-11-18"),
    title: "Uteskole med grilldag",
    description:
      "Vi skal grille. Ta med pølser/toast i aluminiumsfolie, grillspyd De som har, tar med en vedkubbe.",
  },
  {
    date: new Date("2022-12-01"),
    title: "Tur med fadderene",
    description:
      "1AC til Helgerudskogen. 1BD til den store steinen. Vi lager bål. Ta med grillmat og grillpinne. Husk sitteunderlag. Fint hvis de som har mulighet tar med vedkubbe.",
  },
  {
    date: new Date("2022-12-13"),
    title: "Lucia-feiring",
    description:
      "Tirsdag 13. desember skal 1. trinn gå i Lucia-tog. Da ønsker vi at barna kler seg i hvitt. Skolen stiller med lys og glitter. Dere trenger ikke å gå til innkjøp av noe nytt tøy, en stor hvit t-skjorte er også fint.",
  },
];

export { importantDates };
